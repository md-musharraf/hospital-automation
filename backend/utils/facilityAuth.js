...
const { accountKindsFor, legacyModulesFrom } = require('./facilityProfile');
...
const ALL_SCOPES = ['staff', 'doctor', 'lab', 'pharmacy'];
 ...
 */
function scopesForFacility(hospital) {
  if (!hospital) return [];
  const modules =
 ...
 */
function rejectWeakPassword(password) {
 ...
 */
function facilityTokenClaims(hospital) {
  return {
    role: 'facility',
