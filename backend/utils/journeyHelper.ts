...
export function setStage(token: any, stage: string, by?: string): boolean {
  ...
  if (typeof token.markModified === 'function') token.markModified('stageHistory');
  return true;
}

/** Are all requested lab tests finished? (No tests at all counts as done.) */
export function allTestsComplete(token: any): boolean {
  const tests = (token && token.labTests) || [];
  if (tests.length === 0) return true;
  return tests.every((t: any) => t.status === 'Completed');
}

/** Any test still waiting on the lab? */
export function hasPendingTests(token: any): boolean {
  const tests = (token && token.labTests) || [];
  return tests.some((t: any) => t.status !== 'Completed');
}
 ...
 */
export function deriveStage(token: any): string {
...
export function stageMessage(stage: string): string {
  switch (stage) {
    case 'In Consultation':
      return 'You are with the doctor now. / आप अभी डॉक्टर के पास हैं।';
    case 'Lab Pending':
      return 'Please visit the lab for your tests. / कृपया जांच के लिए लैब जाएँ।';
    case 'Lab Complete':
      return 'Your reports are ready — please go back to your doctor. / आपकी रिपोर्ट तैयार है — कृपया डॉक्टर के पास वापस जाएँ।';
    case 'Pharmacy Pending':
      return 'Collect your medicines from the pharmacy counter. / फार्मेसी काउंटर से दवा ले लें।';
    case 'Dispensed':
      return 'Medicines collected. Get well soon! / दवा मिल गई। जल्दी स्वस्थ हों!';
    case 'Completed':
      return 'Your visit is complete. / आपकी विजिट पूरी हो गई।';
    case 'Absent':
      return 'You missed your turn. Please contact reception. / आपकी बारी छूट गई। कृपया रिसेप्शन से संपर्क करें।';
    default:
      return 'You are in the queue. We will alert you when your turn is near. / आप क़तार में हैं। बारी पास आते ही हम बताएंगे।';
  }
}
