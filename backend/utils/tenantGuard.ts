...
export const GUARDED_METHODS: string[] = [
  'count',
  'countDocuments',
  'deleteMany',
  'deleteOne',
 ...
 */
export function isScoped(filter: any): boolean {
 ...
 */
export function guardMode(): string {
 ...
 */
export function tenantGuardPlugin(schema: Schema, { modelName }: { modelName?: string } = {}): void {
  const check = function check(this: any) {
  ...
  GUARDED_METHODS.forEach((method) => schema.pre(method as any, check));
}
