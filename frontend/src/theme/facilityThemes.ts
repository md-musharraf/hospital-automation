...
export const FACILITY_THEMES = {
  hospital: {
    ...
    heroSub:
      'Full-spectrum diagnostics, emergency response and specialist consultations with real-time queue automation.',
  ...
  clinic: {
    ...
    heroSub:
      'Boutique specialist consultations with unhurried attention, warm surroundings and instant token booking.',
  ...
  lab: {
    ...
    pattern: 'grid',
    heroImage:
      'https://images.unsplash.com/photo-1579154204601-01588f351167?q=80&w=1200&auto=format&fit=crop',
    heroKicker: 'Accredited Diagnostics Lab',
    heroTitle: 'Better Diagnostics. Brighter Lives.',
    heroSub:
      'Automated sample processing, rapid turnaround and digital reports — with live counter queue tracking.',
  ...
  medical: {
    ...
    pattern: 'plus',
    heroImage:
      'https://images.unsplash.com/photo-1607619056574-7b8d304a3b6f?q=80&w=1200&auto=format&fit=crop',
    heroKicker: 'Licensed Pharmacy Partner',
    heroTitle: 'Wellness, Ready When You Are',
    heroSub:
      'Genuine medicines, fast prescription refills and doorstep delivery — skip the counter queue entirely.',
  ...
  government: {
    ...
    heroSub:
      'Subsidised, dependable healthcare for every citizen — now with digital tokens and live status updates.',
...
/**
 * The colours `Hospital.primaryColor` / `secondaryColor` are *defaulted* to in
 * the schema. Because Mongoose fills them in for every facility, "the partner
 * chose teal" and "nobody ever picked a colour" are stored identically — so
 * treating any stored value as a deliberate override made every clinic, lab and
 * pharmacy on the platform render in the hospital teal, and the whole category
 * palette below was dead code in practice.
 *
 * Matching the default means "not customised", so the category theme wins. A
 * partner who genuinely wants this exact teal still gets it: it is the hospital
 * category colour, and any other facility type can pick a shade one step away.
 */
const SCHEMA_DEFAULT_COLORS = ['#0d9488', '#0f172a'];

const customised = (value) =>
  typeof value === 'string' &&
  value.trim() !== '' &&
  !SCHEMA_DEFAULT_COLORS.includes(value.trim().toLowerCase());

/**
 * Get the full theme object for a facility. Overlays the facility's own custom
 * primary/secondary colours (white-labelling) on top of the category defaults,
 * so deliberate partner branding always wins — but an untouched schema default
 * does not count as branding.
 */
export function getFacilityTheme(type: any, overrides: any = {}): any {
  const base = FACILITY_THEMES[resolveThemeKey(type)] || FACILITY_THEMES.hospital;
  return {
    ...base,
    primary: customised(overrides.primaryColor) ? overrides.primaryColor : base.primary,
    secondary: customised(overrides.secondaryColor) ? overrides.secondaryColor : base.secondary
  };
}
 ...
 */
export function themeVars(theme: any): any {
  return {
