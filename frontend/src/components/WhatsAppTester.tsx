...
export default function WhatsAppTester({
  ...
  const handleSimulateWhatsAppMessage = async (msgToSend?: string) => {
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-secondary)] font-semibold mt-0.5">
              ...
              <code className="text-emerald-600 dark:text-emerald-400 font-mono font-bold">
                {config?.whatsappNumber || waNumber}
              </code>{' '}
              | Mode:{' '}
              <span className="font-bold text-[var(--primary-color)]">
                {config?.providerMode || 'Auto-Gateway (API Number Active)'}
              </span>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-base font-black text-[var(--text-color)] flex items-center space-x-2">
              ...
              <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-extrabold border border-emerald-500/20">
                Live Sync
              </span>
            </h3>
            <p className="text-xs text-[var(--text-secondary)] font-semibold mt-0.5">
      ...
      {activeTab === 'simulator' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 space-y-4">
            {/* Quick Setup Card */}
            <form
              onSubmit={handleSaveApiNumber}
              className="bg-[var(--bg-color)] p-4 rounded-2xl border border-[var(--border-color)]/30 space-y-3"
            >
              <div>
                ...
                {configSuccess && (
                  <p className="text-[11px] text-emerald-600 font-bold mt-1.5">{configSuccess}</p>
                )}
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar">
              {responseLog.length === 0 ? (
                <div className="text-zinc-500 text-center py-12 font-sans text-xs">
                  <span className="material-symbols-outlined text-[36px] text-zinc-700 mb-2 block">
                    phishing
                  </span>
                ...
                responseLog.map((log, idx) => (
                  <div
                    key={idx}
                    className="bg-zinc-900/80 border border-zinc-800/80 p-3 rounded-xl space-y-2 font-sans"
                  >
                    <div className="flex justify-between text-[10px] text-zinc-400 border-b border-zinc-800/60 pb-1">
                      <span className="font-bold text-emerald-300">
                        USER ({log.sender}): "{log.request}"
                      </span>
                      <span>{log.timestamp}</span>
                    </div>
                    {log.status === 'sending' ? (
                      <p className="text-amber-400 font-mono text-xs animate-pulse">
                      ...
                      <div className="space-y-1">
                        <span className="text-[10px] uppercase font-bold text-zinc-500 tracking-wider">
                          Bot Response (TwiML / Chatbot Engine):
                        </span>
        ...
        <div className="space-y-4 text-xs font-semibold text-[var(--text-secondary)]">
          <div className="bg-[var(--bg-color)] p-4 rounded-2xl border border-[var(--border-color)]/30 space-y-2">
            <h4 className="text-sm font-extrabold text-[var(--text-color)]">Automatic Workflow Triggers</h4>
            <p className="text-xs">
              Providing only the WhatsApp API Number automatically activates instant messaging for all
              hospital operations:
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[var(--bg-color)] p-4 rounded-2xl border border-[var(--border-color)]/30 space-y-3">
              ...
              <p className="text-[11px] text-[var(--text-secondary)]">
                Sends booking confirmation, token number, doctor name, cabin location, and wait time
                immediately when reception registers a walk-in.
              </p>
              <button
            ...
            <div className="bg-[var(--bg-color)] p-4 rounded-2xl border border-[var(--border-color)]/30 space-y-3">
              ...
              <p className="text-[11px] text-[var(--text-secondary)]">
                Dispatches an instant WhatsApp broadcast to the patient when the doctor clicks "Call Next
                Patient" from their cabin panel.
              </p>
              <button
            ...
            <div className="bg-[var(--bg-color)] p-4 rounded-2xl border border-[var(--border-color)]/30 space-y-3">
              ...
              <p className="text-[11px] text-[var(--text-secondary)]">
                Sends high-priority emergency confirmation when staff overrides a ticket to Emergency SOS
                priority.
              </p>
              <button
            ...
            <div className="bg-[var(--bg-color)] p-4 rounded-2xl border border-[var(--border-color)]/30 space-y-3">
              ...
              <p className="text-[11px] text-[var(--text-secondary)]">
                Automated 9:00 AM daily cron job sends follow-up re-visit reminders directly to patients'
                WhatsApp.
              </p>
              <button
              ...
              <button
                onClick={() =>
                  navigator.clipboard.writeText(
                    'https://hospital-automation-wine.vercel.app/api/v1/chat/whatsapp'
                  )
                }
            ...
            <div className="bg-[var(--bg-color)] p-4 rounded-2xl border border-[var(--border-color)]/30 space-y-2">
              ...
              <ol className="list-decimal list-inside space-y-1 text-[11px]">
                ...
                <li>
                  ...
                  <code className="bg-[var(--card-bg)] px-1 rounded font-mono text-[var(--primary-color)]">
                    HTTP POST
                  </code>
                  .
                </li>
