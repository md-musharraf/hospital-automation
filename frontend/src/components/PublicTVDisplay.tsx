    ...
    <div className="flex-grow bg-zinc-955 text-white p-8 flex flex-col justify-between h-[calc(100vh-62px)] relative overflow-hidden font-sans text-left">
      ...
      {calledToken && (
        <div className="absolute inset-0 bg-[#0891b2] flex flex-col items-center justify-center text-center p-8 z-50 animate-fade-in border-4 border-cyan-400 text-left">
          ...
          <h2 className="text-[12vw] font-black leading-none mt-4 text-white tracking-tight uppercase animate-pulse">
            {calledToken.tokenNumber}
          </h2>
          <p className="text-[4vw] font-extrabold mt-6 text-cyan-100 uppercase tracking-widest">
            Proceed to {calledToken.roomName}
          </p>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col space-y-8 text-left">
        ...
        <div className="flex justify-between items-center pb-6 border-b border-white/10">
          <div className="flex items-center space-x-3.5">
            <span className="material-symbols-outlined text-[36px] text-[#0891b2] animate-pulse">
              volume_up
            </span>
            <div>
              <h2 className="text-3xl font-black tracking-tight">CareeAi Waiting Lounge</h2>
              <p className="text-xs text-zinc-400 font-extrabold uppercase tracking-widest mt-0.5">
                Real-time Cabin Admissions
              </p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-2xl font-black text-zinc-305">
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-8">
          {queues.map((q) => {
            ...
            return (
              <div
                key={q._id}
                className="bg-zinc-900 border border-white/5 rounded-3xl p-6 flex flex-col justify-between shadow-2xl relative overflow-hidden"
              >
                ...
                <div className="py-6 border-t border-b border-white/5 my-6 flex flex-col items-center justify-center min-h-[140px]">
                  {isConsulting ? (
                    <div className="text-center">
                      <p className="text-[12px] text-zinc-400 uppercase font-extrabold tracking-widest">
                        Active Patient
                      </p>
                      <h4 className="text-6xl font-black text-[#0891b2] mt-2 tracking-tight uppercase animate-pulse">
                        {q.currentToken.tokenNumber}
                      </h4>
                      <p className="text-xs font-semibold text-zinc-300 mt-2 truncate max-w-[200px]">
                        {q.currentToken.patient?.name}
                      </p>
                    </div>
                  ) : (
                    <div className="text-center text-zinc-500">
                      <span className="material-symbols-outlined text-[36px] text-zinc-650 mb-2">
                        sensor_door
                      </span>
                ...
                <div className="space-y-2.5 text-xs font-semibold">
                  <span className="text-[11px] text-zinc-400 uppercase font-extrabold tracking-widest block mb-1">
                    Queue Status
                  </span>
