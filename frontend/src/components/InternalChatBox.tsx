...
export default function InternalChatBox({ token, user, role }) {
  ...
  useEffect(() => {
    ...
    const handleInternalMsg = (msg) => {
      ...
      if (msg.receiverRole === myRole || msg.senderRole === myRole || msg.receiverRole === 'All') {
    ...
    <div className="bg-[var(--card-bg)] border border-[var(--border-color)]/30 rounded-2xl p-5 shadow-[var(--card-shadow)] flex flex-col h-[400px] w-full max-w-2xl">
      <div className="flex items-center justify-between pb-3 border-b border-[var(--border-color)]/30 mb-4 shrink-0">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="font-extrabold text-[var(--text-color)] text-[15px]">Internal Hospital Intercom</h4>
        </div>
        ...
        <div className="flex items-center space-x-1.5 text-[13px]">
          ...
          <select
            ...
            className="bg-[var(--bg-color)] border border-[var(--border-color)]/50 rounded-lg px-2.5 py-1 font-bold text-[13px] outline-none text-[var(--text-color)]"
          >
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1 mb-4 no-scrollbar">
        {loading ? (
          <div className="h-full flex items-center justify-center text-[13px] text-[var(--text-secondary)] font-bold">
            Syncing intercom connection...
          </div>
        ) : messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center text-[13px] text-[var(--text-secondary)]/50 italic p-6">
            <span className="material-symbols-outlined text-[32px] text-zinc-350 dark:text-zinc-650 mb-2">
              chat_bubble
            </span>
            <p>
              Intercom is silent. Send a message to coordinate with{' '}
              {receiverRole === 'Staff'
                ? 'the Staff Desk'
                : receiverRole === 'Doctor'
                  ? 'Doctors'
          ...
          messages.map((msg, idx) => {
            ...
            return (
              <div key={idx} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                <div
                  className={`max-w-[85%] rounded-xl p-3 text-[13px] shadow-sm ${
                    isMe
                ...
                >
                  <div className="flex items-center justify-between mb-1 text-[11px] font-black text-[var(--text-secondary)] tracking-wider">
                    <span>
                      {msg.senderName} ({msg.senderRole})
                    </span>
                    <span className="ml-4 font-bold text-[11px]">
      ...
      <form onSubmit={handleSendMessage} className="flex items-center space-x-2 shrink-0">
        <input
          ...
          className="flex-1 bg-[var(--bg-color)] border border-[var(--border-color)]/60 focus:border-[var(--primary-color)] rounded-xl px-4 py-2 outline-none text-[13px] text-[var(--text-color)] font-semibold"
        />
        <button
          type="submit"
