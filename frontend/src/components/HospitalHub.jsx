...
export default function HospitalHub() {
  ...
  const filteredHospitals = processedHospitals.filter(
    (h) =>
  ...
  return (
    <div className="flex-1 w-full min-h-screen overflow-y-auto bg-[var(--bg-color)] text-[var(--text-color)] transition-colors duration-200 no-scrollbar">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[var(--primary-color)]/10 via-[var(--bg-color)] to-[var(--bg-color)] py-12 md:py-20 px-6 sm:px-12 border-b border-[var(--border-color)]/25">
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -top-24 -left-24 w-96 h-96 rounded-full bg-[var(--primary-color)]/20 blur-3xl animate-float-slow"
        ></div>
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -bottom-32 right-0 w-[28rem] h-[28rem] rounded-full bg-[var(--tertiary-color)]/15 blur-3xl animate-float-slow"
        ...
        <div className="relative max-w-[1280px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center text-left">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <h1 className="animate-fade-in-up delay-100 text-4xl sm:text-5xl md:text-6xl font-black tracking-tight leading-none">
              ...
              <span className="bg-gradient-to-r from-[var(--primary-color)] to-[var(--tertiary-color)] bg-clip-text text-transparent">
            ...
            <p className="animate-fade-in-up delay-200 text-sm sm:text-base md:text-md text-[var(--text-secondary)] font-medium leading-relaxed max-w-xl">
              Eliminate physical waiting lines, check live cabin statuses, and experience real-time AI-powered
              triage across Hospitals, Clinics, Labs & Medical Stores. Select a partner facility below to book
              instantly.
            </p>
            ...
            <div className="animate-fade-in-up delay-300 flex items-center max-w-xl bg-[var(--card-bg)] border border-[var(--border-color)]/70 rounded-2xl p-2 shadow-lg shadow-black/5 relative group focus-within:border-[var(--primary-color)] transition-all duration-300">
              <div className="flex-1 flex items-center px-2">
                ...
                {searchQuery && (
                  <button
                    ...
                    className="p-1 rounded-full hover:bg-[var(--border-color)]/25 flex items-center"
                  >
              ...
              <button
                ...
                className={`flex items-center justify-center p-2.5 rounded-xl bg-[var(--bg-color)] hover:bg-[var(--primary-color)] hover:text-[var(--primary-text)] transition-all active:scale-95 duration-100 disabled:opacity-50 border border-[var(--border-color)]/30 text-[var(--text-secondary)] ${
                  userCoords
                    ? 'text-[var(--primary-color)] border-[var(--primary-color)]/40 bg-[var(--primary-color)]/5'
                    : ''
                }`}
            ...
            {geoError && <p className="text-xs text-rose-500 font-bold animate-bounce">{geoError}</p>}
            {userCoords && (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 max-w-xl">
                    <div className="flex-1 relative">
                      <span className="material-symbols-outlined absolute left-2.5 top-1/2 -translate-y-1/2 text-[18px] text-[var(--primary-color)] pointer-events-none">
                        public
                      </span>
                      <select
                        value={selectedState}
                      ...
                      <span className="material-symbols-outlined absolute right-2 top-1/2 -translate-y-1/2 text-[18px] text-zinc-400 pointer-events-none">
                        expand_more
                      </span>
                    </div>

                    <div className="flex-1 relative">
                      <span className="material-symbols-outlined absolute left-2.5 top-1/2 -translate-y-1/2 text-[18px] text-[var(--tertiary-color)] pointer-events-none">
                        location_city
                      </span>
                      <select
      ...
      <div className="max-w-[1280px] mx-auto py-16 px-6 sm:px-8">
        <div className="flex justify-between items-center mb-8 text-left">
          <div>
            ...
            <p className="text-xs text-[var(--text-secondary)] font-semibold mt-1">
              Select any hospital, clinic, lab, or medical store to check waiting queues
            </p>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-3">
            <span className="material-symbols-outlined text-[48px] text-orange-600 animate-spin">
              refresh
            </span>
          ...
          <div className="text-center py-20 bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)]/30 p-8 shadow-sm space-y-4">
            ...
            <div>
              <h3 className="text-lg font-black mb-1 text-[var(--text-color)]">
            ...
            {visibleHospitals.map((h) => {
              ...
              return (
                <div
                  ...
                  style={{
                    '--facility-ring': cardTheme.ring,
                    '--primary-color': cardTheme.primary,
                    '--primary-text': '#ffffff'
                  }}
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background: `linear-gradient(90deg, ${cardTheme.primary}, ${cardTheme.accent})`
                    }}
                  />
                  {/* Cover Image */}
                  <div className="h-48 w-full overflow-hidden relative bg-zinc-800">
                    ...
                    <div className="absolute top-4 right-4 flex flex-col items-end space-y-1.5">
                      <span
                        className="text-white text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full shadow-sm flex items-center space-x-1"
                        style={{ background: cardTheme.primary }}
                      >
                        <span className="w-1.5 h-1.5 bg-white rounded-full animate-ping"></span>
                        <span>Active Queue</span>
                      </span>
                      <span
                        className="backdrop-blur-md text-white text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded shadow-sm border border-white/10 flex items-center space-x-1"
                        style={{ background: `${cardTheme.secondary}cc` }}
                      >
                    ...
                    {h.distance !== undefined && (
                      <span className="absolute top-4 left-4 bg-black/60 backdrop-blur-md text-white text-[10px] font-black px-2.5 py-1 rounded-full shadow-sm flex items-center space-x-1">
                        <span className="material-symbols-outlined text-[12px] text-[var(--primary-color)]">
                          near_me
                        </span>
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <div className="mt-auto pt-4 border-t border-[var(--border-color)]/30 flex justify-between items-center gap-2">
                      <button
                        ...
                        className="text-xs font-extrabold px-3 py-2 rounded-xl border border-[var(--border-color)]/50 text-[var(--text-secondary)] hover:border-[var(--primary-color)]/50 hover:text-[var(--primary-color)] flex items-center space-x-1 active:scale-95 duration-100 transition-all"
                      >
                        <span className="material-symbols-outlined text-[15px]">confirmation_number</span>
                        <span>Book Token</span>
                      </button>
                      <button className="bg-[var(--primary-color)] hover:bg-[var(--primary-container)] text-[var(--primary-text)] hover:text-[var(--text-color)] text-xs font-extrabold px-4 py-2 rounded-xl flex items-center space-x-1 shadow-md shadow-[var(--primary-color)]/10 active:scale-95 duration-100 transition-all transition-all-custom">
                        <span>View Facility</span>
                        <span className="material-symbols-outlined text-[14px] group-hover:translate-x-0.5 transition duration-150">
                          arrow_forward
                        </span>
                      </button>
                    </div>
      ...
      <div className="bg-[var(--card-bg)] border-t border-[var(--border-color)]/30 py-12 px-6 sm:px-8 text-center text-xs text-[var(--text-secondary)] font-semibold">
        <div className="max-w-2xl mx-auto space-y-4">
          ...
          <p>
            CareeAi is a registered healthcare SaaS queue management provider. By joining any queue, you agree
            to our Terms of Service and Privacy Policy. Patient information is strictly handled in accordance
