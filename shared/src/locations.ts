...
export const INDIA_STATES: readonly string[] = Object.freeze(
  Object.keys(INDIA_LOCATIONS).sort((a, b) => a.localeCompare(b))
);
...
const DISTRICT_LOOKUP = new Map<string, Map<string, string>>();

for (const [stateName, info] of Object.entries(INDIA_LOCATIONS)) {
 ...
 */
function findCanonicalState(rawState: unknown): string | null {
 ...
 */
export function getDistrictsForState(state: string | null | undefined): string[] {
  if (!state || typeof state !== 'string') return [];
  const canonicalState = findCanonicalState(state);
  if (!canonicalState) return [];
  const info = INDIA_LOCATIONS[canonicalState];
  if (!info) return [];
...
export function getActiveLocations(
  hospitals: Array<{ state?: string; district?: string; city?: string }>
): ActiveLocationsResult {
  const stateCounts = new Map<string, number>();
  const districtCountsByState = new Map<string, Map<string, number>>();
  ...
  for (const h of hospitals || []) {
    ...
    if (resolvedDistrict) {
      if (!districtCountsByState.has(resolvedState)) {
        districtCountsByState.set(resolvedState, new Map<string, number>());
      }
  ...
  const states: ActiveLocationCount[] = Array.from(stateCounts.entries())
    .map(([name, count]) => ({ name, count }))
    .sort((a, b) => a.name.localeCompare(b.name));

  const districts: Record<string, ActiveLocationCount[]> = {};
